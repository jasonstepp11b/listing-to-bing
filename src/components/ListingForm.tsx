...
export function ListingForm({ onSubmit }: Props) {
  ...
  const [leadType, setLeadType] = useState<'buyer' | 'seller'>('seller')
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setErrors({})
    onSubmit({ listing: listing.trim(), city: city.trim(), price: parseFloat(price), leadType })
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-12">
      <div className="mb-8">
        ...
        <p className="text-slate-500">
          Paste a property listing and get a ready-to-run campaign in under 10 seconds.
        </p>
      </div>
          ...
          <div className="flex gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-slate-700">
                Find a seller{' '}
                <span className="text-slate-400">(list their home)</span>
              </span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
              ...
              <span className="text-sm text-slate-700">
                Find a buyer{' '}
                <span className="text-slate-400">(sell the home)</span>
              </span>
            </label>
          </div>
        </div>
