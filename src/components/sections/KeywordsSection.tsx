...
export function KeywordsSection({ keywords }: Props) {
  const hasBuyer = keywords.buyer.length > 0
  const hasSeller = keywords.seller.length > 0
  ...
  return (
    <section className="bg-white border border-slate-200 rounded-lg p-6">
      ...
      <div className="space-y-8">
        {hasBuyer && (
          <KeywordTable list={keywords.buyer} label="Buyer Keywords" />
        )}
        {hasSeller && (
          <KeywordTable list={keywords.seller} label="Seller Keywords" />
        )}
      </div>
    </section>
  )
